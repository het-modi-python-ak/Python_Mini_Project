    ...
    def load_from_file(self):
        ...
        for task_id, task_data in data.items():
            try:
                ...
                print(f"❌ Error processing task ID {task_id} during load: {e}")
                continue

        # Ensure Task._id is higher than the highest loaded ID
        if self.tasks:
            Task._id = max(self.tasks.keys()) + 1

def validate_date(date_text):
    """Validates date format and ensures it is in the future."""
    try:
        date_obj = datetime.strptime(date_text, "%Y-%m-%d %H:%M")
        # Removed future date check to allow tracking past events if needed
...
async def reminder_loop(manager):
    """Prints urgent sorted tasks every 10s and updates overdue tasks."""
    while True:
        await asyncio.sleep(10)

        # --- NEW LOGIC: Check and update overdue tasks ---
        now = datetime.now()
        tasks_to_save = False
        for t in manager.tasks.values():
            if t.status.lower() != "completed":
                try:
                    due_date_obj = datetime.strptime(t.due_date, "%Y-%m-%d %H:%M")
                    if due_date_obj < now:
                        # Automatically mark as completed
                        t.status = "Completed"
                        print(f"\n✅ Task ID {t.id} ('{t.title}') automatically marked as Completed (overdue).")
                        tasks_to_save = True
                except ValueError:
                    # Handle cases where a task might have an invalid date format
                    continue
        
        if tasks_to_save:
            manager.save_to_file() # Save changes to disk after updating statuses

        # --- Existing Logic: Sort and print reminders ---
        pending = [t for t in manager.tasks.values() if t.status.lower() != "completed"]

        # Add exception handling for invalid date formats during sorting
        try:
            pending.sort(key=lambda x: datetime.strptime(x.due_date, "%Y-%m-%d %H:%M"))
        except ValueError as e:
            print(f"\n❌ Error sorting tasks by date: {e}. Check date formats in your tasks.")
            continue
...
async def main():
    ...
    while True:
        ...
        # Added try-except for potential KeyboardInterrupt (Ctrl+C)
        try:
            choice = await asyncio.to_thread(input,'Choice: ')
        except KeyboardInterrupt:
            print("\n:wave: Exiting Task Manager")
            break

        # Handle user menu selections
        if not choice:
            continue # Prevents error if user just hits enter
